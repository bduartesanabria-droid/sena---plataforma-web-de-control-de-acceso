...
export const SenaLogo: React.FC<SenaLogoProps> = ({
  ...
  return (
    <div className={`inline-flex items-center gap-2.5 select-none ${containerClassName}`}>
      {/* Official SENA SVG Emblem */}
      <svg
        viewBox="0 0 75 74"
        fill={fillColor}
        ...
        role="img"
      >
        <path d={SENA_LOGO_PATH} fill={fillColor} />
      </svg>

      {/* Accompanying Institutional Typography */}
      {showText && mode !== 'emblem-only' && (
        <div className={`flex flex-col leading-none ${mode === 'auto' ? 'hidden sm:flex' : 'flex'}`}>
          <div className="flex items-center gap-1.5">
            <span 
              className="font-black tracking-tight text-[#1C7C00] drop-shadow-2xs text-lg sm:text-xl lg:text-2xl"
              style={{ color: fillColor === '#39A900' ? '#1C7C00' : fillColor }}
            >
              SENA
            </span>
          </div>
          {textSubtitle && (
            <span className="text-[10px] lg:text-[11px] font-bold text-slate-500 tracking-wider uppercase mt-0.5 max-w-[170px] truncate">
              {textSubtitle}
            </span>
          )}
        </div>
